...
(function initCatalog() {
  ...
  function openProduct(productId, trigger) {
    ...
    const modalPricePill = modalPrice?.closest(".dialog-pill");
    if (modalPricePill) modalPricePill.hidden = hasVariants;
    if (modalVariants) modalVariants.hidden = !hasVariants;
    if (modalVariantOptions) {
      modalVariantOptions.replaceChildren();
      if (hasVariants) {
        modalVariantOptions.style.display = "grid";
        product.variants.forEach((variant) => {
          ...
          const variantPrice = document.createElement("strong");
          const vp = pricingFor(product, variant.id);
          variantPrice.innerHTML = vp.hasOffer
            ? `<span class="current-product-price sale">${money(vp.finalPrice)}</span><span class="original-product-price">${money(vp.originalPrice)}</span><span class="catalog-offer-chip">خصم ${number(vp.percent)}%</span>`
            : `<span class="current-product-price">${money(vp.finalPrice)}</span>`;
          button.append(variantName, variantPrice);
        ...
        modalVariantOptions.style.display = "none";
      }
    }
